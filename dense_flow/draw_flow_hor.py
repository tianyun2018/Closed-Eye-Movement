...
def findHighSpot(data,fps):
    data = np.array(data)
    x = range(0, len(data))  # X轴数据
    plt.plot(x, data, label="data_ff", color='red', linewidth=2)
    plt.show()
    # 峰值检测并显示
    ...
    if (fps % 2) == 0:
        ...
        fps_odd = fps
    tmp_smooth1 = signal.savgol_filter(data, fps_odd, 5)
    ...
    # 打印峰值个数及对应坐标
    num = len(value[0])
    print("检测眼动左右移动次数：",num)
    print("检测眼动位移最大位置帧数：",value)
    print(signal.argrelextrema(tmp_smooth1, np.greater))
    plt.plot(signal.argrelextrema(tmp_smooth1, np.greater)[0], tmp_smooth1[signal.argrelextrema(tmp_smooth1, np.greater)], 'o')
    plt.plot(signal.argrelextrema(-tmp_smooth1, np.greater)[0], tmp_smooth1[signal.argrelextrema(-tmp_smooth1, np.greater)], '+')
...
if __name__ =="__main__":
    ...
    for i in range(len(result_name_list)):
        ...
        num1_moving_smooth_savgol, num2_moving_smooth_savgol, \
        num1_smooth_moving_savgol, num2_smooth_moving_savgol, \
        moving_average_fps, savgol_filter_fps =  findHighSpot_smooth(acc_flow,video_path,fps,fps//5, video_times)
